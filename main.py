...
def upload_file_to_onedrive(
    ...
    print(f"\nFile uploaded: {folder_path}/{onedrive_file_name}")
...
def upload_pdf_to_onedrive(
    ...
    print(f"\nPDF uploaded: {folder_path}/{onedrive_file_name}")
...
def _set_cell_in_sheet_xml(sheet_xml: str, addr: str, value) -> str:
    ...
    def replace_self_closing(m):
        # Strip type attr and self-closing slash, make it an inlineStr
...
def populate_excel_template_local(
    ...
    # Map of cell address → value for the main sheet
    cell_updates = {
        "D1": current_date,
        "D2": purchase_price,
        "D3": deal,
    ...
    # The main sheet is always xl/worksheets/sheet1.xml
    ...
    print(f"\nExcel populated and saved: {output_path}")
...
def main():
    ...
    pdf_output_name = f"{ticket_value}_Closing_Form.pdf"

    local_template_path = f"temp_template_{ticket_value}.xlsx"
    local_output_path = f"temp_output_{ticket_value}.xlsx"
    ...
    print("\nWaiting for OneDrive to process the file...")
    time.sleep(5)

    onedrive_excel_path = f"{active_folder}/{output_excel_name}"
