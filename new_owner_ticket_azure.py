...
WORK_DIR = Path(os.getenv("TMP", "/tmp"))
...
def get_request_value(req, body, name, default=None):
...
def process_new_owner_ticket(env, ticket_value):
    ...
    logger.info("OneDrive user: %s", runtime["user_email"])
    logger.info("Dataverse URL: %s", runtime["dataverse_url"])
    logger.info("Ticket ID: %s", ticket_value)
    ...
    logger.info("New Owner Ticket Submission generated successfully")

    return {
        "status": "SUCCESS",
        "environment": env,
        "ticket_id": ticket_value,
        "docx_file": output_docx_name,
        "pdf_file": output_pdf_name,
        "active_folder": active_folder,
        "dataverse_pdf_column": NEW_OWNER_PDF_FILE_COLUMN,
    }
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        ticket_value = get_request_value(req, body, "id")

        if not ticket_value:
            raise ValueError("Missing required request parameter: id")

        result = process_new_owner_ticket(env, ticket_value)
        ...
        return func.HttpResponse(
            body=json.dumps({
                "status": "ERROR",
                "message": "HTTP request failed while processing the ticket.",
                "detail": str(error),
            }),
            status_code=502,
            mimetype="application/json",
